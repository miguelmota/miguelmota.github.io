...
function loadLatitudeStream() {
	$('.stream-latitude .loading-error').remove();
	...
	$.ajax({
	    url: 'https://www.googleapis.com/latitude/v1/currentLocation?access_token=ya29.AHES6ZTzQIzsFWeauIRSQdb34bLwD18xA1sWf25lBrOLMcODEEbEAx8&callback=?',
	    type: 'get',
	    dataType: 'json',
	    cache: false,
	    crossDomain: true,
	...
	var timer = setTimeout(function() {
					$('.stream-latitude .loader').css('display','none').after('<span class="loading-error">Could not retrieve location</span>');
				}, 10000);
}
			...
			function(data) {
				...
				$.each(data, function(i, item) {
					...
					if(item["type"] == 'WatchEvent') {
						...
						message = "<strong>"+item.payload["shas"][0][2]+"</strong>";
						to = "to";
					}
