...
var stream = {
	...
	load: {
		...
		lastfm: function (username, count, selector) {
			...
			container = $(selector),
			list = $('<ul class="'+ streamDefaults.listClass +'"></ul>'),
			JSONURL = 'http://ws.audioscrobbler.com/2.0/?callback=?';
			...
			$.getJSON(JSONURL,
					...
					function(data){
						$.each(data.recenttracks.track, function(i, item) {
							...
							image = '/img/social-stream/placeholder-34x34.png',
							listItem;
		...
		stumbleupon: function (username, count, selector) {
			...
			container = $(selector),
			list = $('<ul class="'+ streamDefaults.listClass +'"></ul>'),
			URL = 'http://www.stumbleupon.com/rss/stumbler/miguelmota/likes',
			XMLURL = 'http://moogs.foodfail.org/proxy/contents?url='+URL;
		...
		youtube: function (username, count, selector) {
			...
			container = $(selector),
			list = $('<ul class="'+ streamDefaults.listClass +'"></ul>'),
			JSONURL = 'http://gdata.youtube.com/feeds/api/users/'+ username +'/uploads?alt=json';
