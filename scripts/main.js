		...
		$.getJSON('http://api.wakoopa.com/miguelmota/recently_used.json?callback=?',
			...
			function wakoopaApi(data){
				...
				for(var i = 0; i < data.length; i++){
					...
					html.push("<li><a href='", entry.complete_url, "'>", entry.name, "</a> <time class='status-date'>"+niceTime(date)+"</time>", "</li>");
				}
				html.push("</ul>");
		...
		$.getJSON('http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?',
			{
				...
				function jsonFlickrFeed(data) {
					var htmlString = '<div class="stream-carousel-wrap"><a href="javascript:void(0);" class="stream-carousel-nav stream-carousel-nav-prev">&#171;</a><div class="stream-carousel stream-carousel-flickr"><ul class="stream-ul stream-ul-flickr jcarousel-skin-tango">';
					...
					$('.stream-flickr').append(htmlString +'</ul></div><a href="javascript:void(0);" class="stream-carousel-nav stream-carousel-nav-next">&#187;</a></div>');
...
function portfolioPage(){
	...
	$('.project').live({
		...
		mouseleave:
			function(){
				jQuery('.overlay-popup', this).animate({bottom: '-95px'}, {queue: false, duration: 125});
		  }
	});
...
function contactPage(){
	...
	function success(){
		...
		$('form.contact-form').slideUp(300, function(){
			$('.contact-form-thank-you').html('<p class="thank-you-name">Thank you, <strong>'+name+'</strong>.</p><p>Your message has been successfully sent!<br />I will get in touch with you soon.</p>').fadeIn(1200);
		});
	}
