	...
	$.getJSON('http://twitter.com/status/user_timeline/miguel_mota.json?callback=?', 
			...
			function(data){
				...
				$.each(data, function(i, status){
					...
					var list_item = "<li id='"+id+"' class='status'><span class='post'><a href='http://twitter.com/miguel_mota/status/"+id+"' rel='external'><span class='icon icon-twitter-bird-16'></span> "+post+"</a></span> <time class='status-date' datetime='"+date+"'>"+date+"</time></li>";
	...
	$.getJSON('http://miguelmota.tumblr.com/api/read/json?num=3&callback=?',
			function(data){
				...
				$.each(data.posts, function(i, posts){ 
			    	  	...
			    	  	var list_item = "<li><a href='"+url+"' rel='external'><span class='icon icon-"+type+"-16'></span> "+slug.substring(0,1).toUpperCase()+slug.substr(1,200)+"</a> <time class='status-date' datetime='"+date+"'>"+date+"</time></li>";
			...
			function(data){
				...
				$.each(data, function(i, item){
					...
					var url = item.u;
		    	  	...
		    	  	var list_item = "<li><a href='"+url+"' rel='external'><span class='icon icon-link-16'></span> "+title+"</a> <time class='status-date' datetime='"+date+"'>"+date+"</time></li>";
		...
		$.getJSON('http://ws.audioscrobbler.com/2.0/?callback=?', 
				...
				function(data){   
					...
					$.each(data.recenttracks.track, function(i, item){ 
							...
							var list_item = "<li><a href='"+url+"' rel='external'><img class='stream-thumb' src='"+image+"' alt='' /> "+artist+" - "+name+"</a> <time class='status-date' datetime='"+date+"'>"+date+"</time></li>";
		...
		$.getJSON('http://api.wakoopa.com/miguelmota/recently_used.json?callback=?',
			...
			function wakoopaApi(data){
				...
				for(var i = 0; i < data.length; i++){
					...
					html.push("<li><a href='"+entry.complete_url+"' rel='external'> <img class='stream-thumb' src='"+entry.complete_thumb_url+"' alt='' /> "+entry.name+"</a> <time class='status-date' datetime='"+date+"'>"+date+"</time>", "</li>");
				}
