...
$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});var c=0;$(document).ajaxComplete(function(){var hash=window.location.hash.substr(2);switch(hash){case "":case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;default:break}});
