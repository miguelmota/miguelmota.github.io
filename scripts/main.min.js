...
$("li.status[id='"+id+"'] span").find(":contains('@')").text().substr(1)+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('#')").wrapInner("<a href='http://twitter.com/#!/search/"+$("li.status[id='"+id+"'] span").find(":contains('#')").text().substr(1)+"'> </a>")});$(".stream-twitter div.loader").css("display","none");$(".stream-twitter").append(htmlString+"</ul>");showMtip(".stream-logo-twitter")});$(".stream-facebook div.loader").css("display","block");$.getJSON("https://graph.facebook.com/miguel.mota2/feed?&callback=?",
...
post_id+"'>"+niceTime(date)+"</a></time></li>");break;default:break}});$(".stream-facebook div.loader").css("display","none");showMtip("a.social.facebook")});$(".stream-tumblr div.loader").css("display","block");$.getJSON("http://miguelmota.tumblr.com/api/read/json?callback=?",{num:"3"},function(data){$.each(data.posts,function(i,posts){var htmlString='<ul class="stream-ul stream-ul-tumblr">';var date=(new Date(this["date-gmt"])).toUTCString();var url=this.url;var caption=this["photo-caption"];var slug=
this.slug.replace(/-/g," ");htmlString+="<li>&#187; <a href='"+url+"'>"+slug.substring(0,1).toUpperCase()+slug.substr(1,200)+"</a> <time class='status-date'>"+niceTime(date)+"</time></li>"});$(".stream-tumblr div.loader").css("display","none");$(".stream-tumblr").append(htmlString+"</ul>");showMtip(".stream-logo-tumblr")});$(".stream-delicious div.loader").css("display","block");$.getJSON("http://feeds.delicious.com/v2/json/miguelmota?callback=?",{count:"3"},function(data){$.each(data,function(i,
item){var htmlString='<ul class="stream-ul stream-ul-delicious">';var title=item.d;var url=item.u;var date=(new Date(item.dt)).toUTCString();htmlString+="<li>&#187; <a href='"+url+"'>"+title+"</a> <time class='status-date'>"+niceTime(date)+"</time></li>"});$(".stream-delicious div.loader").css("display","none");$(".stream-delicious").append(htmlString+"</ul>");showMtip(".stream-logo-delicious")});$(".stream-lastfm div.loader").css("display","block");$.getJSON("http://ws.audioscrobbler.com/2.0/?callback=?",
{format:"json",method:"user.getRecentTracks",user:"miguel_mota",api_key:"b25b959554ed76058ac220b7b2e0a026",limit:"5"},function(data){$.each(data.recenttracks.track,function(i,item){var htmlString='<ul class="stream-ul stream-ul-lastfm">';var url=item.url;var name=item.name;var artist=item.artist["#text"];var date=item.date["#text"];htmlString+="<li><a href='"+url+"'>"+artist+" - "+name+"</a> <time class='status-date'>"+date+"</time></li>"});$(".stream-lastfm div.loader").css("display","none");$(".stream-lastfm").append(htmlString+
"</ul>");showMtip(".stream-logo-lastfm")});$(".stream-wakoopa div.loader").css("display","block");$.getJSON("http://api.wakoopa.com/miguelmota/recently_used.json?callback=?",{limit:"3"},function wakoopaApi(data){var html=["<ul class='stream-ul stream-ul-wakoopa'>"];for(var i=0;i<data.length;i++){var entry=data[i].software;var date=(new Date(entry.last_active_at)).toUTCString();html.push("<li><a href='",entry.complete_url,"'>",entry.name,"</a> <time class='status-date'>"+niceTime(date)+"</time>","</li>")}html.push("</ul>");
document.getElementById("stream-wakoopa-software").innerHTML=html.join("");$(".stream-wakoopa div.loader").css("display","none");showMtip(".stream-logo-wakoopa")});$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?",{format:"json",id:"40464790@N08",tags:"outdoors, los angeles, vans warped tour",tagmode:"any"},jsonFlickrFeed);function jsonFlickrFeed(data){var htmlString='<ul class="stream-ul stream-ul-flickr">';$.each(data.items,function(i,item){var thumbnail=item.media.m.replace("_m.jpg",
"_s.jpg");var photo=item.media.m.replace("_m.jpg","_b.jpg");htmlString+="<li><a class='fancybox' rel='flickr' href='"+photo+"' title='"+item.title+"' ><img src='"+thumbnail+"' alt='' /></a></li>";if(i==3)return false});$(".stream-flickr div.loader").css("display","none");$(".stream-flickr").append(htmlString+"</ul>");showMtip(".stream-logo-flickr");$("a.fancybox").fancybox({showCloseButton:false,titlePosition:"inside",titleFormat:formatTitle,padding:0,transitionIn:"none",transitionOut:"none",speedIn:150,
overlayColor:"#000",overlayOpacity:0.8});function formatTitle(title,currentArray,currentIndex,currentOpts){return'<div class="fancybox-title"><span><a href="javascript:void(0);" onclick="$.fancybox.close();">close X</a></span>'+(title&&title.length?"<strong>"+title+"</strong>":"")+"Image "+(currentIndex+1)+" of "+currentArray.length+"</div>"}}c++;refreshStream()}function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)=="stream"||window.location.pathname.substr(1)=="index"||
window.location.pathname=="")$("a#stream").trigger("click")},3E4)}}
