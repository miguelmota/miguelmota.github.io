...
"function")if(window.location.hash)loadPage();else redirectPath();function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=window.location.hash.substr(1);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}if(typeof window.history.pushState!="function"){function redirectPath(){var pathname=window.location.pathname.substr(1);$("ul.nav li a").each(function(){if($(this).attr("href")==
pathname)window.location="./#!"+path})}}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=
$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});var c=0;var bc=0;
$(document).ajaxComplete(function(){if(typeof window.history.pushState!="function")var path=window.location.hash.substr(2);else var path=window.location.pathname.substring(1);switch(path){case "":case "index":case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();break;default:break}if(window.location.pathname.substr(1,4)=="post")while(bc==0){blogPage();$("nav.main a#stream").removeClass("selected");$("nav.main a#blog").addClass("selected");
