...
$("a#blog").addClass("selected");break;default:break}$("ul#nav li a").each(function(){if($(this).attr("href")==pathname);else{streamPage();document.title="Miguel Mota | Freelance Web Developer"}});$("a#"+$("span.sub").text()).addClass("selected");$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");if(typeof window.history.pushState!="function")if(window.location.hash)window.location="/"+window.location.hash.substr(2);$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,
"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});topYloc=parseInt($("a.top").css("top").substring(0,
$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});
