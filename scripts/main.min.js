...
else if(window.location.pathname.substring(1,5)=="post")$("#blog").addClass("selected");else $("#stream").addClass("selected");streamPage();alert(window.location.pathname.substring(1,5));$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";
window.location.hash=window.location.hash.substr(1);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,
