...
if(typeof window.history.pushState!="function")if(window.location.hash=="#!")window.location="/";alert(pathname);$("ul.nav li a").each(function(){if($(this).attr("href")==pathname){alert(pathname);switch($(this).attr("id")){case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();alert("asdf");break;default:break}}else{streamPage();document.title="Miguel Mota | Freelance Web Developer"}});$("a#"+$("span.sub").text()).addClass("selected");
$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");if(typeof window.history.pushState!="function")if(window.location.hash)window.location="/"+window.location.hash.substr(2);$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=
