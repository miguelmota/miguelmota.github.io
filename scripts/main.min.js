...
niceTime(date)+"</time>","</li>")}html.push("</ul>");document.getElementById("wakoopa-software").innerHTML=html.join("");$("div.wakoopa div.loader").css("display","none");showMtip("a.social.wakoopa")});$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?",{format:"json",id:"40464790@N08",tags:"outdoors, los angeles, vans warped tour",tagmode:"any"},displayImages);function displayImages(data){var htmlString='<ul class="flickr-ul">';$.each(data.items,function(i,item){var sourceSquare=
item.media.m.replace("_m.jpg","_s.jpg");htmlString+="<li><a class='fancybox' rel='miguelmota' href='"+item.link+"' title='"+item.title+"' ><img src='"+sourceSquare+"' alt='' /></a></li>";if(i==3)return false});$(".flickr-stream div.loader").css("display","none");$(".flickr-stream").append(htmlString+"</ul>");showMtip(".flickr-logo")}c++;refreshStream()}function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)=="stream"||window.location.pathname.substr(1)=="index"||window.location.pathname==
"")$("a#stream").trigger("click")},3E4)}$("a.fancybox").fancybox({showCloseButton:false,titlePosition:"inside",titleFormat:formatTitle,padding:0,transitionIn:"none",transitionOut:"none",speedIn:150,overlayColor:"#000",overlayOpacity:0.8});function formatTitle(title,currentArray,currentIndex,currentOpts){return'<div class="fancybox-title"><span><a href="javascript:void(0);" onclick="$.fancybox.close();">close X</a></span>'+(title&&title.length?"<strong>"+title+"</strong>":"")+"Image "+(currentIndex+
1)+" of "+currentArray.length+"</div>"}}
