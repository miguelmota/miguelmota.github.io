...
b],f.body["scroll"+b],f.documentElement["scroll"+b],f.body["offset"+b],f.documentElement["offset"+b]):e===A?parseFloat(c.css(f,d)):this.css(d,typeof e==="string"?e:e+"px")}})})(window);(function($){$.fn.mtip=function(){$(this).each(function(){$(this).attr("mtip",$(this).attr("title")).removeAttr("title");var title=$(this).attr("mtip");$(this).wrap("<div class='tooltip-container' />");$(this).css("white-space","nowrap");$("div.tooltip-container").prepend("<div class='tooltip' />");$(this).hover(function(){$(this).prev().text(title).stop(true,true).animate({opacity:"show",top:"-30"},"slow")},function(){$(this).prev().animate({opacity:"hide",top:"-20"},"fast")})})}})(jQuery);(function($){$.extend($.fn,{validate:function(options){if(!this.length){options&&options.debug&&window.console&&console.warn("nothing selected, can't validate, returning nothing");return}var validator=$.data(this[0],"validator");if(validator)return validator;validator=new $.validator(options,this[0]);$.data(this[0],"validator",validator);if(validator.settings.onsubmit){this.find("input, button").filter(".cancel").click(function(){validator.cancelSubmit=true});if(validator.settings.submitHandler)this.find("input, button").filter(":submit").click(function(){validator.submitButton=
...
e)}});$.extend($.fn,{validateDelegate:function(delegate,type,handler){return this.bind(type,function(event){var target=$(event.target);if(target.is(delegate))return handler.apply(target,arguments)})}})})(jQuery);(function(a){var m,t,u,f,D,h,E,n,z,A,q=0,e={},o=[],p=0,c={},l=[],I=null,v=new Image,J=/\.(jpg|gif|png|bmp|jpeg)(.*)?$/i,W=/[^\.]\.(swf)\s*$/i,K,L=1,y=0,s="",r,j,i=false,B=a.extend(a("<div/>")[0],{prop:0}),M=a.browser.msie&&a.browser.version<7&&!window.XMLHttpRequest,N=function(){t.hide();v.onerror=v.onload=null;I&&I.abort();m.empty()},O=function(){if(false===e.onError(o,q,e)){t.hide();i=false}else{e.titleShow=false;e.width="auto";e.height="auto";m.html('<p id="fancybox-error">The requested content cannot be loaded.<br />Please try again later.</p>');
...
enableKeyboardNav:true,onStart:function(){},onCancel:function(){},onComplete:function(){},onCleanup:function(){},onClosed:function(){},onError:function(){}};a(document).ready(function(){a.fancybox.init()})})(jQuery);jQuery.extend({easing:{backEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;var s=1.70158;return c*(p/=1)*p*((s+1)*p-s)+firstNum},backEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;var s=1.70158;return c*((p=p/1-1)*p*((s+1)*p+s)+1)+firstNum},backEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;var s=1.70158;if((p/=0.5)<1)return c/2*p*p*(((s*=1.525)+1)*p-s)+firstNum;else return c/2*((p-=2)*p*(((s*=1.525)+1)*p+s)+2)+firstNum},bounceEaseIn:function(p,n,firstNum,diff){var c=firstNum+
diff;var inv=this.bounceEaseOut(1-p,1,0,diff);return c-inv+firstNum},bounceEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;if(p<1/2.75)return c*7.5625*p*p+firstNum;else if(p<2/2.75)return c*(7.5625*(p-=1.5/2.75)*p+0.75)+firstNum;else if(p<2.5/2.75)return c*(7.5625*(p-=2.25/2.75)*p+0.9375)+firstNum;else return c*(7.5625*(p-=2.625/2.75)*p+0.984375)+firstNum},circEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return-c*(Math.sqrt(1-(p/=1)*p)-1)+firstNum},circEaseOut:function(p,n,firstNum,
diff){var c=firstNum+diff;return c*Math.sqrt(1-(p=p/1-1)*p)+firstNum},circEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;if((p/=0.5)<1)return-c/2*(Math.sqrt(1-p*p)-1)+firstNum;else return c/2*(Math.sqrt(1-(p-=2)*p)+1)+firstNum},cubicEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return c*(p/=1)*p*p+firstNum},cubicEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return c*((p=p/1-1)*p*p+1)+firstNum},cubicEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;if((p/=0.5)<
1)return c/2*p*p*p+firstNum;else return c/2*((p-=2)*p*p+2)+firstNum},elasticEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;if(p==0)return firstNum;if(p==1)return c;var peroid=0.25;var s;var amplitude=c;if(amplitude<Math.abs(c)){amplitude=c;s=peroid/4}else s=peroid/(2*Math.PI)*Math.asin(c/amplitude);return-(amplitude*Math.pow(2,10*(p-=1))*Math.sin((p*1-s)*2*Math.PI/peroid))+firstNum},elasticEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;if(p==0)return firstNum;if(p==1)return c;var peroid=
0.25;var s;var amplitude=c;if(amplitude<Math.abs(c)){amplitude=c;s=peroid/4}else s=peroid/(2*Math.PI)*Math.asin(c/amplitude);return-(amplitude*Math.pow(2,-10*p)*Math.sin((p*1-s)*2*Math.PI/peroid))+c},expoEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return p==0?firstNum:c*Math.pow(2,10*(p-1))+firstNum-c*0.001},expoEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return p==1?c:diff*1.001*(-Math.pow(2,-10*p)+1)+firstNum},expoEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;
if(p==0)return firstNum;if(p==1)return c;if((p/=0.5)<1)return c/2*Math.pow(2,10*(p-1))+firstNum-c*5.0E-4;else return c/2*1.0005*(-Math.pow(2,-10*--p)+2)+firstNum},quadEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return c*(p/=1)*p+firstNum},quadEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return-c*(p/=1)*(p-2)+firstNum},quadEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;if((p/=0.5)<1)return c/2*p*p+firstNum;else return-c/2*(--p*(p-2)-1)+firstNum},quartEaseIn:function(p,
n,firstNum,diff){var c=firstNum+diff;return c*(p/=1)*p*p*p+firstNum},quartEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return-c*((p=p/1-1)*p*p*p-1)+firstNum},quartEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;if((p/=0.5)<1)return c/2*p*p*p*p+firstNum;else return-c/2*((p-=2)*p*p*p-2)+firstNum},quintEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return c*(p/=1)*p*p*p*p+firstNum},quintEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return c*((p=p/1-1)*p*p*p*p+1)+
firstNum},quintEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;if((p/=0.5)<1)return c/2*p*p*p*p*p+firstNum;else return c/2*((p-=2)*p*p*p*p+2)+firstNum},sineEaseIn:function(p,n,firstNum,diff){var c=firstNum+diff;return-c*Math.cos(p*(Math.PI/2))+c+firstNum},sineEaseOut:function(p,n,firstNum,diff){var c=firstNum+diff;return c*Math.sin(p*(Math.PI/2))+firstNum},sineEaseInOut:function(p,n,firstNum,diff){var c=firstNum+diff;return-c/2*(Math.cos(Math.PI*p)-1)+firstNum}}});$(document).ready(function(){if(screen.width<=640){$("h1.title").insertAfter(".nav-logo",function(){$(".sub").remove()});$("#cse-search-form").insertAfter(".blog-description")}$("body").fadeIn(650,function(){$("div.ie-bar").slideDown("slow")});$("div.ie-bar a.ie-bar-close").click(function(){$("div.ie-bar").slideUp("normal")});if(typeof window.history.pushState!="function")if(window.location.hash=="#!")window.location="/";$("a[href^=http]").live("click",function(){window.open(this.href);return false});
$("ul.main-nav li a").each(function(){if($(this).attr("id")==$("section.content").attr("class").split(" ")[1]){$(this).addClass("selected");switch($("section.content").attr("class").split(" ")[1]){case "stream":streamPage();break;case "about":break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();default:break}}});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");if(typeof window.history.pushState!=
"function")if(window.location.hash)window.location="/"+window.location.hash.substr(2);$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});$(window).scroll(function(){var scrollTop=$(document).scrollTop();
scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top-link").animate({top:offset},{duration:500,queue:false})});topYloc=parseInt($("a.top-link").css("top").substring(0,$("a.top-link").css("top").indexOf("px")));$.getScript("http://platform.tumblr.com/v1/share.js");var tumblr_link_url=document.getElementById("post-link").getAttribute("href");var tumblr_link_name=document.getElementById("post-link").innerText;var tumblr_link_description=document.getElementById("post-content").innerText;
var tumblr_button=document.getElementById("tumblr-share-button");tumblr_button.setAttribute("href","http://www.tumblr.com/share/link?url="+encodeURIComponent(tumblr_link_url)+"&name="+encodeURIComponent(tumblr_link_name)+"&description="+encodeURIComponent(tumblr_link_description));tumblr_button.setAttribute("title",tumblr_link_name);tumblr_button.setAttribute("style","display:inline-block; text-indent:-9999px; overflow:hidden; width:81px; height:20px; background:url('http://platform.tumblr.com/v1/share_1.png') top left no-repeat transparent;")});
$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top-link").fadeIn(600);if(window.pageYOffset<200)$("a.top-link").fadeOut(185)});var bsc=0;
$(document).ajaxComplete(function(){while(c==0){if(typeof window.history.pushState!="function")var path2=window.location.pathname.substr(1);else var path2=window.location.pathname.substr(1);var title=$("h1.title span.sub:first").text()+$("div.theTitle").text();$("ul.main-nav a").removeClass("selected");while(bsc==0){if($("section.content").attr("class").split(" ")[2]=="post"||$("section.content").attr("class").split(" ")[2]=="archive")$("ul.main-nav a#blog").addClass("selected");else $("ul.main-nav a").removeClass("selected");
bsc++}if(path2!="")$("ul.main-nav a#"+path2).addClass("selected");else $("ul.main-nav a#stream").addClass("selected");switch(path2){case "":case "index":case "stream":streamPage();break;case "portfolio":case "portfolio/":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();break;default:break}c++}});var c=0;var bc=0;var ldc=0;var pathname=window.location.pathname;
var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+" ago";return amount}}();
$("ul.main-nav a:not('.nav-logo, #blog')").live("click",function(){if(typeof window.history.pushState=="function"&&screen.width>640){c=0;bc=0;ldc=0;var loadSubTitle=$(this).attr("href")+" span.sub";var toLoad=$(this).attr("href")+" section.content";$("h1.title span.sub").fadeOut("fast");$("section.content").fadeOut("fast",loadContent);$("div.loader-container").fadeIn("normal");function loadContent(){$("h1.title span.sub").load(loadSubTitle);$("section.content").load(toLoad,showNewContent)}if(typeof window.history.pushState==
"function"){var stateObj={foo:$(this).attr("href")};history.pushState(stateObj,"Title",$(this).attr("href"))}else window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);return false}else return true});var position=0;var length="portfolio".length;
...
function showNewContent(){if(typeof window.history.pushState!="function");$("h1.title span.sub").show();$("section.content, footer.main").fadeIn("normal",hideLoader)}function hideLoader(){$("div.loader-container").hide()}function showMtip(element){var link=element;$(link).trigger("mouseenter");setTimeout(function(){$(link).trigger("mouseleave")},3E3)}
function streamPage(){$("a.social:not('.latitude, .blog-social')").text("");showMtip("a.blog-social");showMtip("a.social.latitude");while(c==0){$(".mtip").mtip();$("div.twitter div.loader").css("display","block");$.getJSON("http://twitter.com/status/user_timeline/miguel_mota.json?&count=5&callback=?",function(data){$.each(data,function(i,status){var post=status.text;var id=status.id_str;var date=(new Date(status.created_at)).toUTCString();var newText="<span>"+post.split(" ").join("</span> <span>")+
"</span>";$("ul.twitter-status").append("<li id='"+id+"' class='status'>&#187; <span class='post'>"+newText+"</span> <time class='status-date'><a href='http://twitter.com/miguel_mota/status/"+id+"'>"+niceTime(date)+"</a></time></li>");$("li.status[id='"+id+"'] span").find(":contains('http')").wrapInner("<a href='"+$("li.status[id='"+id+"'] span").find(":contains('http')").text()+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('@')").wrapInner("<a href='http://twitter.com/"+$("li.status[id='"+
id+"'] span").find(":contains('@')").text().substr(1)+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('#')").wrapInner("<a href='http://twitter.com/#!/search/"+$("li.status[id='"+id+"'] span").find(":contains('#')").text().substr(1)+"'> </a>")});$("div.twitter div.loader").css("display","none");showMtip("a.social.twitter")});$("div.facebook div.loader").css("display","block");$.getJSON("https://graph.facebook.com/miguel.mota2/feed?limit=3&callback=?",function(json){$.each(json.data,function(i,
fb){var post=fb.message;var post_id=fb.id.substr(16);var type=fb.type;var link=fb.link;var name=fb.name;var caption=fb.caption;var description=fb.description;var date=(new Date(fb.created_time)).toUTCString();switch(type){case "status":$("ul.facebook-status").append("<li class='status'>&#187; <span class='post'>"+post+"</span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");break;case "link":if(post)$("ul.facebook-status").append("<li class='status'>&#187; Link: <span class='post'>"+
post+" <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");else $("ul.facebook-status").append("<li class='status'>&#187; Link: <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");break;case "video":if(post)$("ul.facebook-status").append("<li class='status'>&#187; Video: <span class='post'>"+
post+" <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");else $("ul.facebook-status").append("<li class='status'>&#187; Video: <span class='post'><a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");break;default:break}});$("div.facebook div.loader").css("display","none");
showMtip("a.social.facebook")});$("div.tumblr div.loader").css("display","block");$.getJSON("http://miguelmota.tumblr.com/api/read/json?num=3&callback=?",function(data){$.each(data.posts,function(i,posts){var date=(new Date(this["date-gmt"])).toUTCString();var url=this.url;var caption=this["photo-caption"];var slug=this.slug.replace(/-/g," ");$("ul.tumblr-posts").append("<li>&#187; <a href='"+url+"'>"+slug.substring(0,1).toUpperCase()+slug.substr(1,200)+"</a> <time class='status-date'>"+niceTime(date)+
"</time></li>")});$("div.tumblr div.loader").css("display","none");showMtip("a.social.tumblr")});$("div.delicious div.loader").css("display","block");$.getJSON("http://feeds.delicious.com/v2/json/miguelmota?count=3&callback=?",function(data){$.each(data,function(i,item){var title=item.d;var url=item.u;var date=(new Date(item.dt)).toUTCString();$("ul.delicious-bookmarks").append("<li>&#187; <a href='"+url+"'>"+title+"</a> <time class='status-date'>"+niceTime(date)+"</time></li>")});$("div.delicious div.loader").css("display",
"none");showMtip("a.social.delicious")});$("div.lastfm div.loader").css("display","block");$.getJSON("http://ws.audioscrobbler.com/2.0/?method=user.getRecentTracks&user=miguel_mota&api_key=b25b959554ed76058ac220b7b2e0a026&format=json&limit=5&callback=?",function(data){$.each(data.recenttracks.track,function(i,item){var url=item.url;var name=item.name;var artist=item.artist["#text"];var date=item.date["#text"];$("ul.lastfm-tracks").append("<li><a class='link' href='"+url+"'>"+artist+" - "+name+"</a> <time class='status-date'>"+
date+"</time></li>")});$("div.lastfm div.loader").css("display","none");showMtip("a.social.lastfm")});$("div.wakoopa div.loader").css("display","block");$.getJSON("http://api.wakoopa.com/miguelmota/recently_used.json?limit=3&callback=?",function wakoopaApi(data){var html=["<ul class='wakoopa-software'>"];for(var i=0;i<data.length;i++){var entry=data[i].software;var date=(new Date(entry.last_active_at)).toUTCString();html.push("<li><a class='link' href='",entry.complete_url,"'>",entry.name,"</a> <time class='status-date'>"+
niceTime(date)+"</time>","</li>")}html.push("</ul>");document.getElementById("wakoopa-software").innerHTML=html.join("");$("div.wakoopa div.loader").css("display","none");showMtip("a.social.wakoopa")});c++;refreshStream()}function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)=="stream"||window.location.pathname.substr(1)=="index"||window.location.pathname=="")$("a#stream").trigger("click")},3E4)}}
function portfolioPage(){$(".work-sort-all").live("click",function(){$(".work-sort-wrap a").removeClass("selected");$(this).addClass("selected");$(".project").slideDown("fast")});$(".work-sort-web").live("click",function(){$(".work-sort-wrap a").removeClass("selected");$(this).addClass("selected");$(".project:not('.project-web')").slideUp("fast");$(".project-web").slideDown("fast")});$(".work-sort-identity").live("click",function(){$(".work-sort-wrap a").removeClass("selected");$(this).addClass("selected");
$(".project:not('.project-identity')").slideUp("fast");$(".project-identity").slideDown("fast")});$(".work-sort-code").live("click",function(){$(".work-sort-wrap a").removeClass("selected");$(this).addClass("selected");$(".project:not('.project-code')").slideUp("fast");$(".project-code").slideDown("fast")});$(".project .image-container").live({mouseenter:function(){jQuery("span.overlay",this).fadeOut(200);$(this).css({"-webkit-box-shadow":"0 0 10px #fff","-moz-box-shadow":"0 0 10px #fff","box-shadow":"0 0 10px #fff"})},
mouseleave:function(){jQuery("span.overlay",this).hide().fadeIn(300);$(this).css({"-webkit-box-shadow":"2px 2px 5px #111","-moz-box-shadow":"2px 2px 5px #111","box-shadow":"2px 2px 5px #111"})}});$(".project .image-container").live({mouseenter:function(){jQuery(".overlay-popup",this).stop().clearQueue().animate({bottom:"0"},{queue:false,duration:200,easing:"cubicEaseOut"})},mouseleave:function(){jQuery(".overlay-popup",this).animate({bottom:"-90px"},{queue:false,duration:200,easing:"bounceEaseOut"})}});
$("a.fancybox").hover(function(){jQuery("span.zoom-icon",this).css("display","block")},function(){jQuery("span.zoom-icon",this).hide()});$("a.fancybox").fancybox({showCloseButton:false,titlePosition:"inside",titleFormat:formatTitle,padding:0,transitionIn:"none",transitionOut:"none",speedIn:150,overlayColor:"#000",overlayOpacity:0.8});function formatTitle(title,currentArray,currentIndex,currentOpts){return'<div class="fancybox-title"><span><a href="javascript:void(0);" onclick="$.fancybox.close();">close X</a></span>'+
(title&&title.length?"<strong>"+title+"</strong>":"")+"Image "+(currentIndex+1)+" of "+currentArray.length+"</div>"}}
function contactPage(){$("a.contact-submit").live("click",function(){$.validator.addMethod("namecheck",function(value,element){return this.optional(element)||/^[a-zA-Z]+?\s?[a-zA-Z]+?\s?[a-zA-Z]+$/.test(value)});$("form.contact-form").validate({rules:{name:{namecheck:true,required:true},email:{required:true,email:true},message:{required:true,minlength:10}},messages:{name:{namecheck:"",required:""},email:{required:"",email:""},message:"",minlength:""},onkeyup:true,debug:true});if($("form.contact-form").valid()==
true){var str=$("form.contact-form").serialize();$.ajax({type:"get",url:"http://miguelmota.webuda.com/contact/contact.php",data:str,success:function(){success()},error:function(){$("form.contact-form").slideUp(300);$("a.contact-submit").html("<span>sending...</span>");setTimeout(function(){$("form.contact-form").html("<p>Sorry, there was an error. Message was not sent.</p>")},300)}});return false}else return false});function success(){$("form.contact-form").slideUp(300);$("a.contact-submit").html("<span>sending...</span>");
setTimeout(function(){$("form.contact-form").html("<p>Thank you <strong>"+$("input#name").val()+"</strong>, <br />Your message has been successfully sent!<br />I will get in touch with you soon.</p>").fadeIn(1200)},300)}}tc=0;
function blogPage(){while(tc==0){$.getScript("http://platform.twitter.com/widgets.js");(function(){var po=document.createElement("script");po.type="text/javascript";po.async=true;po.src="https://apis.google.com/js/plusone.js";var s=document.getElementsByTagName("script")[0];s.parentNode.insertBefore(po,s)})();tc++}while(ldc=0)ldc++}function loadDisqus(){}function loadSearch(){}function displayYear(){var date=new Date;var thisYear=date.getFullYear();document.write(thisYear)}
function displayURL(){document.write(pathname)};