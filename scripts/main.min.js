...
function streamPage(){$("div.stream a.social:not('.latitude')").text("");showMtip("a.social.latitude");while(c==0){$(".mtip").mtip();$("div.twitter div.loader").css("display","block");$.getJSON("http://twitter.com/status/user_timeline/miguel_mota.json?&count=5&callback=?",function(data){$.each(data,function(i,status){var post=status.text;var id=status.id_str;var date=(new Date(status.created_at)).toUTCString();var newText="<span>"+post.split(" ").join("</span> <span>")+"</span>";$("ul.twitter-status").append("<li id='"+
id+"' class='status'>&#187; <span class='post'>"+newText+"</span> <time class='status-date'><a href='http://twitter.com/miguel_mota/status/"+id+"'>"+niceTime(date)+"</a></time></li>");$("li.status[id='"+id+"'] span").find(":contains('http')").wrapInner("<a href='"+$("li.status[id='"+id+"'] span").find(":contains('http')").text()+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('@')").wrapInner("<a href='http://twitter.com/"+$("li.status[id='"+id+"'] span").find(":contains('@')").text().substr(1)+
"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('#')").wrapInner("<a href='http://twitter.com/#!/search/"+$("li.status[id='"+id+"'] span").find(":contains('#')").text().substr(1)+"'> </a>")});$("div.twitter div.loader").css("display","none");showMtip("a.social.twitter")});$("div.facebook div.loader").css("display","block");$.getJSON("https://graph.facebook.com/miguel.mota2/feed?limit=3&callback=?",function(json){$.each(json.data,function(i,fb){var post=fb.message;var post_id=fb.id.substr(16);
var type=fb.type;var link=fb.link;var name=fb.name;var caption=fb.caption;var description=fb.description;var date=(new Date(fb.created_time)).toUTCString();switch(type){case "status":$("ul.facebook-status").append("<li class='status'>&#187; <span class='post'>"+post+"</span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");break;case "link":if(post)$("ul.facebook-status").append("<li class='status'>&#187; Link: <span class='post'>"+
...
mouseleave:function(){jQuery("span.overlay",this).hide().fadeIn(300);$(this).css({"-webkit-box-shadow":"2px 2px 5px #111","-moz-box-shadow":"2px 2px 5px #111","box-shadow":"2px 2px 5px #111"})}});$("a.fancybox").hover(function(){jQuery("span.zoom-icon",this).css("display","block")},function(){jQuery("span.zoom-icon",this).hide()});$("a.fancybox").fancybox({showCloseButton:false,titlePosition:"inside",titleFormat:formatTitle,padding:0,transitionIn:"none",transitionOut:"none",speedIn:150,overlayColor:"#000",
overlayOpacity:0.8});function formatTitle(title,currentArray,currentIndex,currentOpts){return'<div class="fancybox-title"><span><a href="javascript:void(0);" onclick="$.fancybox.close();">close X</a></span>'+(title&&title.length?"<strong>"+title+"</strong>":"")+"Image "+(currentIndex+1)+" of "+currentArray.length+"</div>"}}
