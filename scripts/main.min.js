...
"/";$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");var path=window.location.pathname.substring(1);if(window.location.hash){var domain=document.domain;if(domain=="www.miguelmota.com");}else;function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=window.location.hash.substr(1);
$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}function redirectPath(){var pathname=window.location.pathname.substr(1);switch(pathname){case "":case "index":case "stream":case "portfolio":case "contact":window.location="./#!"+path;default:break}}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},
{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});
...
$("nav.main a.blah").live("click",function(){c=0;var toLoad=$(this).attr("href")+" section.content";window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false});
...
var $=function(s){return document.getElementById(s)},links=$("nav"),output=$("article"),template="",data={first:{name:"Remy",location:"Brighton, UK"},second:{name:"John",location:"San Francisco, USA"},third:{name:"Jeff",location:"Vancover, Canada"},fourth:{name:"Simon",location:"London, UK"}};function reportData(data){output.innerHTML=template.replace(/(:?\{(.*?)\})/g,function(a,b,c){return data[c]})}
addEvent(links,"click",function(event){var title;event.preventDefault();if(event.target.nodeName=="A"){title=event.target.innerHTML;data[title].url=event.target.getAttribute("href");history.pushState(data[title],title,event.target.href);reportData(data[title])}$("#article").load(event.target.getAttribute("href")+" section.content")});