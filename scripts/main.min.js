...
enableKeyboardNav:true,onStart:function(){},onCancel:function(){},onComplete:function(){},onCleanup:function(){},onClosed:function(){},onError:function(){}};a(document).ready(function(){a.fancybox.init()})})(jQuery);$(document).ready(function(){if(screen.width<=640){$("h1.title").insertAfter(".nav-logo",function(){$(".sub").remove()});$("#cse-search-form").insertAfter(".blog-description")}$("body").fadeIn(650,function(){$("div.ie-bar").slideDown("slow")});$("div.ie-bar a.ie-bar-close").click(function(){$("div.ie-bar").slideUp("normal")});if(typeof window.history.pushState!="function")if(window.location.hash=="#!")window.location="/";$("a[rel*=internal]").live("click",function(e){e.preventDefault();return false});
$("a[href^=http]:not([rel*=internal])").live("click",function(){window.open(this.href);return false});initializeFancybox();$("ul.main-nav li a").each(function(){if($(this).attr("id")==$("section.content").attr("class").split(" ")[1]){$(this).addClass("selected");switch($("section.content").attr("class").split(" ")[1]){case "stream":streamPage();break;case "about":break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();default:break}}});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");
if(typeof window.history.pushState!="function")if(window.location.hash)window.location="/"+window.location.hash.substr(2);$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});$(window).scroll(function(){var scrollTop=
$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top-link").animate({top:offset},{duration:500,queue:false})});topYloc=parseInt($("a.top-link").css("top").substring(0,$("a.top-link").css("top").indexOf("px")));$.getScript("http://platform.tumblr.com/v1/share.js");var tumblr_link_url=document.getElementById("post-link").getAttribute("href");var tumblr_link_name=document.getElementById("post-link").innerText;var tumblr_link_description=document.getElementById("post-content").innerText;
...
$(document).ajaxComplete(function(){initializeFancybox();while(c==0){if(typeof window.history.pushState!="function")var path2=window.location.pathname.substr(1);else var path2=window.location.pathname.substr(1);var title=$("h1.title span.sub:first").text()+$("div.theTitle").text();$("ul.main-nav a").removeClass("selected");while(bsc==0){if($("section.content").attr("class").split(" ")[2]=="post"||$("section.content").attr("class").split(" ")[2]=="archive")$("ul.main-nav a#blog").addClass("selected");
else $("ul.main-nav a").removeClass("selected");bsc++}if(path2!="")$("ul.main-nav a#"+path2).addClass("selected");else $("ul.main-nav a#stream").addClass("selected");switch(path2){case "":case "index":case "stream":streamPage();break;case "portfolio":case "portfolio/":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();break;default:break}c++}});
function initializeFancybox(){$("a.fancybox").fancybox({showCloseButton:false,titlePosition:"inside",titleFormat:formatTitle,padding:0,transitionIn:"none",transitionOut:"none",speedIn:150,overlayColor:"#000",overlayOpacity:0.8});function formatTitle(title,currentArray,currentIndex,currentOpts){return'<div class="fancybox-title"><span><a class="button" href="javascript:void(0);" onclick="$.fancybox.close();">close X</a></span>'+(title&&title.length?"<strong>"+title+"</strong>":"")+"Image "+(currentIndex+
1)+" of "+currentArray.length+"</div>"}}var c=0;var bc=0;var ldc=0;var pathname=window.location.pathname;var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+" ago";return amount}}();
...
function streamPage(){$(".stream-logo:not('.stream-logo-blog ,.stream-logo-latitude')").text("");showMtip(".stream-logo-blog");showMtip(".stream-logo-latitude");while(c==0){$(".mtip").mtip();$(".stream-twitter div.loader").css("display","block");$.getJSON("http://twitter.com/status/user_timeline/miguel_mota.json?callback=?",{count:"5"},function(data){$.each(data,function(i,status){var htmlString='<ul class="stream-ul stream-ul-twitter">';var post=status.text;var id=status.id_str;var date=(new Date(status.created_at)).toUTCString();
var newText="<span>"+post.split(" ").join("</span> <span>")+"</span>";htmlString+="<li id='"+id+"' class='status'>&#187; <span class='post'>"+newText+"</span> <time class='status-date'><a href='http://twitter.com/miguel_mota/status/"+id+"'>"+niceTime(date)+"</a></time></li>";$("li.status[id='"+id+"'] span").find(":contains('http')").wrapInner("<a href='"+$("li.status[id='"+id+"'] span").find(":contains('http')").text()+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('@')").wrapInner("<a href='http://twitter.com/"+
$("li.status[id='"+id+"'] span").find(":contains('@')").text().substr(1)+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('#')").wrapInner("<a href='http://twitter.com/#!/search/"+$("li.status[id='"+id+"'] span").find(":contains('#')").text().substr(1)+"'> </a>");$(".stream-twitter").append(htmlString+"</ul>")});$(".stream-twitter div.loader").css("display","none");showMtip(".stream-logo-twitter")});$(".stream-facebook div.loader").css("display","block");$.getJSON("https://graph.facebook.com/miguel.mota2/feed?&callback=?",
{limit:"3"},function(json){$.each(json.data,function(i,fb){var post=fb.message;var post_id=fb.id.substr(16);var type=fb.type;var link=fb.link;var name=fb.name;var caption=fb.caption;var description=fb.description;var date=(new Date(fb.created_time)).toUTCString();switch(type){case "status":$("ul.facebook-status").append("<li class='status'>&#187; <span class='post'>"+post+"</span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");
break;case "link":if(post)$("ul.facebook-status").append("<li class='status'>&#187; Link: <span class='post'>"+post+" <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");else $("ul.facebook-status").append("<li class='status'>&#187; Link: <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+
"</a></time></li>");break;case "video":if(post)$("ul.facebook-status").append("<li class='status'>&#187; Video: <span class='post'>"+post+" <a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></time></li>");else $("ul.facebook-status").append("<li class='status'>&#187; Video: <span class='post'><a href='"+link+"'>"+name+"</a></span> <time class='status-date'><a href='http://www.facebook.com/miguel.mota2/posts/"+
post_id+"'>"+niceTime(date)+"</a></time></li>");break;default:break}});$(".stream-facebook div.loader").css("display","none");showMtip("a.social.facebook")});$(".stream-tumblr div.loader").css("display","block");$.getJSON("http://miguelmota.tumblr.com/api/read/json?num=3&callback=?",{num:"3"},function(data){$.each(data.posts,function(i,posts){var htmlString='<ul class="stream-ul stream-ul-tumblr">';var date=(new Date(this["date-gmt"])).toUTCString();var url=this.url;var caption=this["photo-caption"];
var slug=this.slug.replace(/-/g," ");htmlString+="<li>&#187; <a href='"+url+"'>"+slug.substring(0,1).toUpperCase()+slug.substr(1,200)+"</a> <time class='status-date'>"+niceTime(date)+"</time></li>";$(".stream-tumblr").append(htmlString+"</ul>")});$(".stream-tumblr div.loader").css("display","none");showMtip(".stream-logo-tumblr")});$(".stream-delicious div.loader").css("display","block");$.getJSON("http://feeds.delicious.com/v2/json/miguelmota?callback=?",{count:"3"},function(data){$.each(data,function(i,
item){var htmlString='<ul class="stream-ul stream-ul-delicious">';var title=item.d;var url=item.u;var date=(new Date(item.dt)).toUTCString();htmlString+="<li>&#187; <a href='"+url+"'>"+title+"</a> <time class='status-date'>"+niceTime(date)+"</time></li>";$(".stream-delicious").append(htmlString+"</ul>")});$(".stream-delicious div.loader").css("display","none");showMtip(".stream-logo-delicious")});$(".stream-lastfm div.loader").css("display","block");$.getJSON("http://ws.audioscrobbler.com/2.0/?callback=?",
{format:"json",method:"user.getRecentTracks",user:"miguel_mota",api_key:"b25b959554ed76058ac220b7b2e0a026",limit:"5"},function(data){$.each(data.recenttracks.track,function(i,item){var htmlString='<ul class="stream-ul stream-ul-lastfm stream-ul-chart">';var url=item.url;var name=item.name;var artist=item.artist["#text"];var date=item.date["#text"];htmlString+="<li><a href='"+url+"'>"+artist+" - "+name+"</a> <time class='status-date'>"+date+"</time></li>";$(".stream-lastfm").append(htmlString+"</ul>")});
$(".stream-lastfm div.loader").css("display","none");showMtip(".stream-logo-lastfm")});$(".stream-wakoopa div.loader").css("display","block");$.getJSON("http://api.wakoopa.com/miguelmota/recently_used.json?callback=?",{limit:"3"},function wakoopaApi(data){var html=["<ul class='stream-ul stream-ul-wakoopa stream-ul-chart'>"];for(var i=0;i<data.length;i++){var entry=data[i].software;var date=(new Date(entry.last_active_at)).toUTCString();html.push("<li><a href='",entry.complete_url,"'>",entry.name,
"</a> <time class='status-date'>"+niceTime(date)+"</time>","</li>")}html.push("</ul>");document.getElementById("stream-wakoopa-software").innerHTML=html.join("");$(".stream-wakoopa div.loader").css("display","none");showMtip(".stream-logo-wakoopa")});$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?",{format:"json",id:"40464790@N08",tags:"outdoors, los angeles, vans warped tour",tagmode:"any"},jsonFlickrFeed);function jsonFlickrFeed(data){var htmlString='<a href="javacript:void(0);" class="prev">prev</a><ul class="stream-ul stream-ul-flickr jcarousel-skin-tango">';
$.each(data.items,function(i,item){var thumbnail=item.media.m;var thumbnail_small=item.media.m.replace("_m.jpg","_s.jpg");var photo=item.media.m.replace("_m.jpg","_b.jpg");htmlString+="<li><a class='fancybox' rel='flickr internal' href='"+photo+"' title='"+item.title+"' ><img src='"+thumbnail+"' alt='' /></a></li>";if(i==19)return false});$(".stream-flickr div.loader").css("display","none");$(".stream-flickr").append(htmlString+'</ul><a href="javacript:void(0);" class="next">next</a>');showMtip(".stream-logo-flickr");
$(".stream-ul-flickr").jCarouselLite({btnNext:".next",btnPrev:".prev"})}c++;refreshStream()}function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)=="stream"||window.location.pathname.substr(1)=="index"||window.location.pathname=="")$("a#stream").trigger("click")},3E4)}}
...
"-moz-box-shadow":"2px 2px 5px #111","box-shadow":"2px 2px 5px #111"})}});$(".project").live({mouseenter:function(){jQuery(".overlay-popup",this).animate({bottom:"0"},{queue:false,duration:150})},mouseleave:function(){jQuery(".overlay-popup",this).animate({bottom:"-95px"},{queue:false,duration:125})}});$("a.fancybox").hover(function(){jQuery("span.zoom-icon",this).css("display","block")},function(){jQuery("span.zoom-icon",this).hide()})}
...
true){var str=$("form.contact-form").serialize();$.ajax({type:"get",url:"http://www.foodfail.org/miguelmota/contact.php",data:str,success:function(){success()},error:function(){$("a.contact-submit").html("<span>sending...</span>");$("form.contact-form").slideUp(300,function(){$(".contact-form-thank-you").html("<p>Sorry, there was an error. Message was not sent.</p><p>Email <a href='mailto:hello@miguelmota.com'>hello@miguelmota.com</a>?</p>")})}});return false}else return false});function success(){$("a.contact-submit").html("<span>sending...</span>");
var name=$("input#name").val();$("form.contact-form").slideUp(300,function(){$(".contact-form-thank-you").html('<p class="thank-you-name">Thank you, <strong>'+name+"</strong>.</p><p>Your message has been successfully sent!<br />I will get in touch with you soon.</p>").fadeIn(1200)})}}tc=0;
