...
$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});var c=0;
$(document).ajaxComplete(function(){var path=window.location.pathname.substring(1);switch(path){case "":case "index":case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();break;default:break}if(window.location.pathname.substr(1,4)=="post"){blogPage();$("nav.main a#stream").removeClass("selected");$("nav.main a#blog").addClass("selected")}});
$("nav.main a").live("click",function(){c=0;var toLoad=$(this).attr("href")+" section.content";$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}var stateObj={foo:$(this).attr("href")};history.pushState(stateObj,"Title",$(this).attr("href"));return false});
