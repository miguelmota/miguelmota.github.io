...
"</a> <time class='status-date'>"+niceTime(date)+"</time>","</li>")}html.push("</ul>");document.getElementById("stream-wakoopa-software").innerHTML=html.join("");$(".stream-wakoopa div.loader").css("display","none");showMtip(".stream-logo-wakoopa")});$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?",{format:"json",id:"40464790@N08",tags:"outdoors, los angeles, vans warped tour",tagmode:"any"},jsonFlickrFeed);function jsonFlickrFeed(data){var htmlString='<ul class="stream-ul stream-ul-flickr">';
$.each(data.items,function(i,item){var thumbnail=item.media.m;var thumbnail_small=item.media.m.replace("_m.jpg","_s.jpg");var photo=item.media.m.replace("_m.jpg","_b.jpg");htmlString+="<li><a class='fancybox' rel='flickr internal' href='"+photo+"' title='"+item.title+"' ><img src='"+thumbnail+"' alt='' /></a></li>";if(i==3)return false});$(".stream-flickr div.loader").css("display","none");$(".stream-flickr").append(htmlString+"</ul>");showMtip(".stream-logo-flickr")}c++;refreshStream()}function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)==
"stream"||window.location.pathname.substr(1)=="index"||window.location.pathname=="")$("a#stream").trigger("click")},3E4)}}
...
true){var str=$("form.contact-form").serialize();$.ajax({type:"get",url:"http://www.foodfail.org/miguelmota/contact.php",data:str,success:function(){success()},error:function(){$("a.contact-submit").html("<span>sending...</span>");var message=$("input#message").val();$("form.contact-form").after('<div class="contact-form-error"></div>');$("form.contact-form").slideUp(300,function(){$("contact-form-error").html("<p>Sorry, there was an error. Message was not sent.</p><p>Email <a href='mailto:hello@miguelmota.com?body="+
message+"'>hello@miguelmota.com</a>?</p>")})}});return false}else return false});function success(){$("a.contact-submit").html("<span>sending...</span>");var name=$("input#name").val();$("form.contact-form").after('<div class="contact-form-you"></div>');$("form.contact-form").slideUp(300,function(){$("contact-form-thank-you").html("<p>Thank you <strong>"+name+"</strong>, <br />Your message has been successfully sent!<br />I will get in touch with you soon.</p>").fadeIn(1200)})}}tc=0;
