...
else $("#stream").addClass("selected");streamPage();$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=window.location.hash.substr(1);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");
function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+
"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});var c=0;
...
$("div.posti h3 a").live("click",function(){var toLoad2=$(this).attr("href")+" div.posti";$("div.posti").fadeOut("fast",loadContent2);$("div.loader").fadeIn("normal");var stateObj2={foo2:$(this).attr("href")};history.pushState(stateObj2,"Title2",$(this).attr("href"));var title2=$("div.posti h3 a").text()+$("div.theTitle").text();document.title=title2;function loadContent2(){$("div.posti").load(toLoad2,showNewContent2);function showNewContent2(){$("div.posti").fadeIn("normal",hideLoader);loadDisqus()}
}return false});function loadDisqus(){var disqus_shortname="miguelmota";var disqus_url="http://wwww.miguelmota.com/{{ page.url }}";(function(){var dsq=document.createElement("script");dsq.type="text/javascript";dsq.async=true;dsq.src="http://"+disqus_shortname+".disqus.com/embed.js";(document.getElementsByTagName("head")[0]||document.getElementsByTagName("body")[0]).appendChild(dsq)})()}var position=0;var length="portfolio".length;
