...
$("nav.main a.9").live("click",function(){c=0;var toLoad=$(this).attr("href")+" section.content";window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false});
...
1?"s":"")+" ago";return amount}}();html.push("<li><a class='link' href='",entry.complete_url,"'>",entry.name,"</a> <span class='date'>"+niceTime(date)+"</span>","</li>")}html.push("</ul>");document.getElementById("wakoopa_software").innerHTML=html.join("");$("div.wakoopa img.loader").css("display","none")});c++}}function refreshStream(){setTimeout(function(){if(window.location.hash.substr(2)=="stream"||window.location.hash.substr(2)=="index"||window.location.hash.substr(1)=="");},3E4)}
