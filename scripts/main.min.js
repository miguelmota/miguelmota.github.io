...
pathname)$(this).addClass("selected")});streamPage();if(pathname==null)$("nav.main a#stream").addClass("selected");$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=window.location.hash.substr(1);$("section.content").fadeOut("fast",
loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&
$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});var c=0;
