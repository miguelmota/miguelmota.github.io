...
enableKeyboardNav:true,onStart:function(){},onCancel:function(){},onComplete:function(){},onCleanup:function(){},onClosed:function(){},onError:function(){}};a(document).ready(function(){a.fancybox.init()})})(jQuery);$(document).ready(function(){if(window.innerWidth<=640){$("div#wrap").prepend("<a id='mobile_bar' href='/mobile'>view mobile site</a><a id='mobile_close' href='javascript:void(0);'>x</a>");$("a#mobile_close").live("click",function(){$(this).hide();$("a#mobile_bar").slideUp("normal")})}$("div#ie a#close").click(function(){$("div#ie").slideUp("normal")});$("body").fadeIn(650,function(){$("div#ie").slideDown("slow")});if(window.location.hash=="")streamPage();if(window.location.hash=="#!")window.location=
"/";$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");var path=window.location.pathname.substring(1);if(window.location.hash){var domain=document.domain;if(domain=="www.miguelmota.com")loadPage()}else redirectPath();function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=
window.location.hash.substr(1);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}function redirectPath(){var pathname=window.location.pathname.substr(1);switch(pathname){case "":case "index":case "stream":case "portfolio":case "contact":window.location="./#!"+path;default:break}}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=
topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});
$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185)});var c=0;$(document).ajaxComplete(function(){var hash=window.location.hash.substr(2);switch(hash){case "":case "index":case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;default:break}});
<<<<<<< HEAD
$("nav.main a").live("click",function(){c=0;var toLoad=$(this).attr("href")+" section.content";window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false});
=======
$("nav.main a.blahblahblahblah").live("click",function(){c=0;var toLoad=$(this).attr("href")+" section.content";window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false});
>>>>>>> parent of bd2da9f... ...
$("div.posti h3 a").live("click",function(){var toLoad2=$(this).attr("href")+" div.posti";window.location.hash="!"+$(this).attr("href").substr(1,$(this).attr("href").length);$("div.posti").fadeOut("fast",loadContent2);$("div.loader").fadeIn("normal");function loadContent2(){$("div.posti").load(toLoad2,showNewContent2);function showNewContent2(){$("div.posti").fadeIn("normal",hideLoader);if(window.location.hash=="#!");loadDisqus()}}return false});
function loadDisqus(){var disqus_shortname="miguelmota";var disqus_url="http://wwww.miguelmota.com/{{ page.url }}";(function(){var dsq=document.createElement("script");dsq.type="text/javascript";dsq.async=true;dsq.src="http://"+disqus_shortname+".disqus.com/embed.js";(document.getElementsByTagName("head")[0]||document.getElementsByTagName("body")[0]).appendChild(dsq)})()}var position=0;var length="portfolio".length;
...
function showNewContent(){$("nav.main a").removeClass("selected");$("#"+window.location.hash.substr(2)).addClass("selected");$("section.content, footer.main").fadeIn("normal",hideLoader);var title=$("span.sub").text()+$("div.theTitle").text();document.title=title;if(window.location.hash=="#!")document.title=theTitle}function hideLoader(){$("div.loader").hide()}
...
1?"s":"")+" ago";return amount}}();html.push("<li><a class='link' href='",entry.complete_url,"'>",entry.name,"</a> <span class='date'>"+niceTime(date)+"</span>","</li>")}html.push("</ul>");document.getElementById("wakoopa_software").innerHTML=html.join("");$("div.wakoopa img.loader").css("display","none")});c++}}
function refreshStream(){setTimeout(function(){if(window.location.hash.substr(2)=="stream"||window.location.hash.substr(2)=="index"||window.location.hash.substr(1)=="")$("a#stream").trigger("click")},3E4)}
...
300)}}google.load("search","1",{language:"en"});function loadSearch(){var customSearchControl=new google.search.CustomSearchControl("013110027163283765539:a-en5rcxdcu");customSearchControl.setResultSetSize(google.search.Search.FILTERED_CSE_RESULTSET);customSearchControl.draw("cse")}function blogPage(){loadSearch()}
var $=function(s){return document.getElementById(s)},examples=$("nav"),output=$("article"),template="",data={first:{name:"Remy",location:"Brighton, UK"},second:{name:"John",location:"San Francisco, USA"},third:{name:"Jeff",location:"Vancover, Canada"},fourth:{name:"Simon",location:"London, UK"}};function reportData(data){output.innerHTML=template.replace(/(:?\{(.*?)\})/g,function(a,b,c){return data[c]})}
addEvent(examples,"click",function(event){var title;event.preventDefault();if(event.target.nodeName=="A"){title=event.target.innerHTML;data[title].url=event.target.getAttribute("href");history.pushState(data[title],title,event.target.href);reportData(data[title])}$("#article").load(event.target.getAttribute("href")+" section.content")});