...
b],f.body["scroll"+b],f.documentElement["scroll"+b],f.body["offset"+b],f.documentElement["offset"+b]):e===A?parseFloat(c.css(f,d)):this.css(d,typeof e==="string"?e:e+"px")}})})(window);(function($){$.fn.mtip=function(options){var settings={"background-color":"#000"};return this.each(function(){if(options)$.extend(settings,options);$(this).wrap('<div class="tooltip_container" />');$(this).hover(function(){$(this).after("<div class='tooltip'></div>");var title=$(this).attr("title");$("div.tooltip").text(title).hide().css("background-color","#000");$("div.tooltip").css("visibility","visible");jQuery(this).next("div.tooltip").stop(true,true).animate({opacity:"show",top:"-30"},"slow")},
function(){jQuery(this).next("div.tooltip").animate({opacity:"hide",top:"-20"},"fast")})})}})(jQuery);(function($){$.extend($.fn,{validate:function(options){if(!this.length){options&&options.debug&&window.console&&console.warn("nothing selected, can't validate, returning nothing");return}var validator=$.data(this[0],"validator");if(validator)return validator;validator=new $.validator(options,this[0]);$.data(this[0],"validator",validator);if(validator.settings.onsubmit){this.find("input, button").filter(".cancel").click(function(){validator.cancelSubmit=true});if(validator.settings.submitHandler)this.find("input, button").filter(":submit").click(function(){validator.submitButton=
...
e)}});$.extend($.fn,{validateDelegate:function(delegate,type,handler){return this.bind(type,function(event){var target=$(event.target);if(target.is(delegate))return handler.apply(target,arguments)})}})})(jQuery);(function(au,k){var Q={version:"3.0.3"};var J=navigator.userAgent.toLowerCase();if(J.indexOf("windows")>-1||J.indexOf("win32")>-1)Q.isWindows=true;else if(J.indexOf("macintosh")>-1||J.indexOf("mac os x")>-1)Q.isMac=true;else if(J.indexOf("linux")>-1)Q.isLinux=true;Q.isIE=J.indexOf("msie")>-1;Q.isIE6=J.indexOf("msie 6")>-1;Q.isIE7=J.indexOf("msie 7")>-1;Q.isGecko=J.indexOf("gecko")>-1&&J.indexOf("safari")==-1;Q.isWebKit=J.indexOf("applewebkit/")>-1;var ab=/#(.+)$/,af=/^(light|shadow)box\[(.*?)\]/i,
...
0,aX=aY.length;a2<aX;a2++)aG(aW,aY[a2],aZ);return aG.filter(a0,aZ)};return aG}();Q.lang={code:"en",of:"of",loading:"loading",cancel:"Cancel",next:"Next",previous:"Previous",play:"Play",pause:"Pause",close:"Close",errors:{single:'You must install the <a href="{0}">{1}</a> browser plugin to view this content.',shared:'You must install both the <a href="{0}">{1}</a> and <a href="{2}">{3}</a> browser plugins to view this content.',either:'You must install either the <a href="{0}">{1}</a> or the <a href="{2}">{3}</a> browser plugin to view this content.'}};
...
K+=">"+ ++aI+"</a>"}}else K=[Q.current+1,Q.lang.of,aN].join(" ")}ad("sb-counter").innerHTML=K;aO()}function U(aH){var K=ad("sb-title-inner"),aG=ad("sb-info-inner"),S=0.35;K.style.visibility=aG.style.visibility="";if(K.innerHTML!="")N(K,"marginTop",0,S);N(aG,"marginTop",0,S,aH)}function av(aG,aM){var aK=ad("sb-title"),K=ad("sb-info"),aH=aK.offsetHeight,aI=K.offsetHeight,aJ=ad("sb-title-inner"),aL=ad("sb-info-inner"),S=aG?0.35:0;N(aJ,"marginTop",aH,S);N(aL,"marginTop",aI*-1,S,function(){aJ.style.visibility=
...
"hidden";aa.style.display="none";ay(true)};if(ao)N(ae,"opacity",0,Q.options.fadeDuration,K);else K()};T.onPlay=function(){r("play",false);r("pause",true)};T.onPause=function(){r("pause",false);r("play",true)};T.onWindowResize=function(){if(!m)return;aB();var K=Q.player,S=ak(K.height,K.width);ar(S.width,S.left);ac(S.innerHeight,S.top);if(K.onWindowResize)K.onWindowResize()};Q.skin=T;au.Shadowbox=Q})(window);(function(a){var m,t,u,f,D,h,E,n,z,A,q=0,e={},o=[],p=0,c={},l=[],I=null,v=new Image,J=/\.(jpg|gif|png|bmp|jpeg)(.*)?$/i,W=/[^\.]\.(swf)\s*$/i,K,L=1,y=0,s="",r,j,i=false,B=a.extend(a("<div/>")[0],{prop:0}),M=a.browser.msie&&a.browser.version<7&&!window.XMLHttpRequest,N=function(){t.hide();v.onerror=v.onload=null;I&&I.abort();m.empty()},O=function(){if(false===e.onError(o,q,e)){t.hide();i=false}else{e.titleShow=false;e.width="auto";e.height="auto";m.html('<p id="fancybox-error">The requested content cannot be loaded.<br />Please try again later.</p>');
...
enableKeyboardNav:true,onStart:function(){},onCancel:function(){},onComplete:function(){},onCleanup:function(){},onClosed:function(){},onError:function(){}};a(document).ready(function(){a.fancybox.init()})})(jQuery);jQuery.fn.pjax=function(container,options){var $=jQuery;if(options)options.container=container;else options=$.isPlainObject(container)?container:{container:container};return this.live("click",function(event){if(event.which==2||event.metaKey)return true;var defaults={url:this.href,container:$(this).attr("data-pjax")};$.pjax($.extend({},defaults,options));event.preventDefault()})};
jQuery.pjax=function(options){var $=jQuery,$container=$(options.container);var defaults={timeout:650,push:true,replace:false,cache:false,type:"GET",dataType:"html",beforeSend:function(xhr){xhr.setRequestHeader("X-PJAX","true")},error:function(){window.location=options.url},success:function(data){if(!$.trim(data)||/<html/i.test(data))return window.location=options.url;$container.html(data);if(!$.pjax.active){$.pjax.active=true;window.history.replaceState({pjax:true},document.title,location.pathname)}var title=
$.trim($container.find("title").remove().text());if(title)document.title=title;var state={pjax:options.container};if(options.data)state.url=options.url+"?"+$.param(options.data);if(options.replace)window.history.replaceState(state,document.title,options.url);else if(options.push)window.history.pushState(state,document.title,options.url);if((options.replace||options.push)&&window._gaq)_gaq.push(["_trackPageview"]);success.apply(this,arguments)}};var success=options.success||$.noop;delete options.success;
options=$.extend(true,{},defaults,options);var xhr=$.ajax(options);$(document).trigger("pjax",xhr,options);return xhr};jQuery.pjax.active=false;jQuery.pjax.firstLoad=true;if(jQuery.browser.webkit&&parseInt(jQuery.browser.version)<534)jQuery.pjax.firstLoad=false;
jQuery(window).bind("popstate",function(event){if(jQuery.pjax==jQuery.noop)return;if(jQuery.pjax.firstLoad)return jQuery.pjax.firstLoad=false;var state=event.state;if(jQuery.pjax.active||state&&state.pjax){var container=jQuery(state.pjax+"");if(container.length)jQuery.pjax({url:state.url||location.href,container:container,push:false});else window.location=location.href}});jQuery.event.props.push("state");if(!window.history||!window.history.pushState){jQuery.pjax=jQuery.noop;jQuery.fn.pjax=function(){return this}};$(document).ready(function(){if(window.innerWidth<=640){$("div#wrap").prepend("<a id='mobile_bar' href='/mobile'>view mobile site</a><a id='mobile_close' href='javascript:void(0);'>x</a>");$("a#mobile_close").live("click",function(){$(this).hide();$("a#mobile_bar").slideUp("normal")})}$("div#ie a#close").click(function(){$("div#ie").slideUp("normal")});$("body").fadeIn(650,function(){$("div#ie").slideDown("slow")});streamPage();$("nav.main a#stream").addClass("selected");var pathname=window.location.pathname;
$("ul.nav li a").each(function(){if($(this).attr("href")==pathname){$("nav.main a#stream").removeClass("selected");$(this).addClass("selected")}});$("nav.main a#logo").live("click",function(){$("nav.main a").removeClass("selected");$("nav.main a#stream").addClass("selected")});$("body").append("<div class='theTitle' style='display: none;'> &#8212; Miguel Mota | Freelance Web Developer</div>");function loadPage(){var toLoad=window.location.hash.substr(2)+" section.content";window.location.hash=window.location.hash.substr(1);
$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}return false}$(window).scroll(function(){var scrollTop=$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$("a.top").animate({top:offset},{duration:500,queue:false})});$("a[href*=#]").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var $target=
$(this.hash);$target=$target.length&&$target||$("[name="+this.hash.slice(1)+"]");if($target.length){var targetOffset=$target.offset().top;$("html,body").animate({scrollTop:targetOffset},1E3);return false}}});topYloc=parseInt($("a.top").css("top").substring(0,$("a.top").css("top").indexOf("px")))});$(window).scroll(function(){if(window.pageYOffset>=200)$("a.top").fadeIn(600);if(window.pageYOffset<200)$("a.top").fadeOut(185);$("a").pjax("#main").live("click",function(){$(this).showLoader()})});
var c=0;var bc=0;
$(document).ajaxComplete(function(){var path=window.location.pathname.substring(1);switch(path){case "":case "index":case "stream":streamPage();break;case "portfolio":portfolioPage();break;case "contact":contactPage();break;case "blog":blogPage();break;default:break}if(window.location.pathname.substr(1,4)=="post")while(bc==0){blogPage();$("nav.main a#stream").removeClass("selected");$("nav.main a#blog").addClass("selected");bc++}if(window.location.pathname.substr(1)=="archive"){$("nav.main a#stream").removeClass("selected");$("nav.main a#blog").addClass("selected")}});
$("nav.main a").live("click",function(){c=0;bc=0;var toLoad=$(this).attr("href")+" section.content";$("section.content").fadeOut("fast",loadContent);$("div.loader").fadeIn("normal");function loadContent(){$("section.content").load(toLoad,showNewContent)}var stateObj={foo:$(this).attr("href")};history.pushState(stateObj,"Title",$(this).attr("href"));return false});
$("div.posti h3 a").live("click",function(){var toLoad2=$(this).attr("href")+" div.posti";$("div.posti").fadeOut("fast",loadContent2);$("div.loader").fadeIn("normal");var stateObj2={foo2:$(this).attr("href")};history.pushState(stateObj2,"Title2",$(this).attr("href"));var title2=$(this).text()+$("div.theTitle").text();document.title=title2;function loadContent2(){$("div.posti").load(toLoad2,showNewContent2);function showNewContent2(){$("div.posti").fadeIn("normal",hideLoader);loadDisqus()}}return false});
function loadDisqus(){var disqus_shortname="miguelmota";var disqus_url="http://wwww.miguelmota.com/{{ page.url }}";(function(){var dsq=document.createElement("script");dsq.type="text/javascript";dsq.async=true;dsq.src="http://"+disqus_shortname+".disqus.com/embed.js";(document.getElementsByTagName("head")[0]||document.getElementsByTagName("body")[0]).appendChild(dsq)})();(function(){var s=document.createElement("script");s.async=true;s.type="text/javascript";s.src="http://"+disqus_shortname+".disqus.com/count.js";
(document.getElementsByTagName("HEAD")[0]||document.getElementsByTagName("body")[0]).appendChild(s)})()}var position=0;var length="portfolio".length;
function textticker(){$("a#stream").text("stream".substring(0,position));$("a#about").text("about".substring(0,position));$("a#portfolio").text("portfolio".substring(0,position));$("a#contact").text("contact".substring(0,position));$("a#blog").text("blog".substring(0,position));if(position++==length)setTimeout("textticker()",1E3);else setTimeout("textticker()",60)}
function showNewContent(){$("nav.main a").removeClass("selected");$("#"+window.location.pathname.substr(1)).addClass("selected");$("section.content, footer.main").fadeIn("normal",hideLoader);var title=$("span.sub").text()+$("div.theTitle").text();document.title=title}function hideLoader(){$("div.loader").hide()}
function streamPage(){$(".mtip").mtip();while(c==0){$("div.facebook img.loader").css("display","block");$.getJSON("https://graph.facebook.com/miguel.mota2/feed?limit=3&callback=?",function(json){$.each(json.data,function(i,fb){var post=fb.message;var post_id=fb.id.substr(16);var type=fb.type;var link=fb.link;var name=fb.name;var caption=fb.caption;var description=fb.description;var date=(new Date(fb.created_time)).toUTCString();var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,
week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+" ago";return amount}}();switch(type){case "status":$("ul.facebook_status").append("<li class='status'>&#187; <span class='post'>"+post+"</span> <span class='date'><a href='http://www.facebook.com/miguel.mota2/posts/"+
post_id+"'>"+niceTime(date)+"</a></span></li>");break;case "link":if(post)$("ul.facebook_status").append("<li class='status'>&#187; Link: <span class='post'>"+post+" <a href='"+link+"'>"+name+"</a></span> <span class='date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></span></li>");else $("ul.facebook_status").append("<li class='status'>&#187; Link: <a href='"+link+"'>"+name+"</a></span> <span class='date'><a href='http://www.facebook.com/miguel.mota2/posts/"+
post_id+"'>"+niceTime(date)+"</a></span></li>");break;case "video":$("ul.facebook_status").append("<li class='status'>&#187; Video: <span class='post'>"+post+" <a href='"+link+"'>"+name+"</a></span> <span class='date'><a href='http://www.facebook.com/miguel.mota2/posts/"+post_id+"'>"+niceTime(date)+"</a></span></li>");break;default:break}});$("div.facebook img.loader").css("display","none")});$("div.twitter img.loader").css("display","block");$.getJSON("http://twitter.com/status/user_timeline/miguel_mota.json?&count=5&callback=?",
function(data){$.each(data,function(i,status){var post=status.text;var id=status.id_str;var date=(new Date(status.created_at)).toUTCString();var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+
" ago";return amount}}();var newText="<span>"+post.split(" ").join("</span> <span>")+"</span>";$("ul.twitter_status").append("<li id='"+id+"' class='status'>&#187; <span class='post'>"+newText+"</span> <span class='date'><a href='http://twitter.com/miguel_mota/status/"+id+"'>"+niceTime(date)+"</a></span></li>");$("li.status[id='"+id+"'] span").find(":contains('http')").wrapInner("<a href='"+$("li.status[id='"+id+"'] span").find(":contains('http')").text()+"'> </a>");$("li.status[id='"+id+"'] span").find(":contains('@')").wrapInner("<a href='http://twitter.com/"+
$("li.status[id='"+id+"'] span").find(":contains('@')").text().substr(1)+"'> </a>")});$("div.twitter img.loader").css("display","none")});$("div.tumblr img.loader").css("display","block");$.getJSON("http://miguelmota.tumblr.com/api/read/json?num=3&callback=?",function(data){$.each(data.posts,function(i,posts){var date=(new Date(this["date-gmt"])).toUTCString();var url=this.url;var caption=this["photo-caption"];var slug=this.slug.replace(/-/g," ");var niceTime=function(){var ints={second:1,minute:60,
hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+" ago";return amount}}();$("ul.tumblr_posts").append("<li>&#187; <a href='"+url+"'>"+slug.substring(0,1).toUpperCase()+slug.substr(1,200)+"</a> <span class='date'>"+niceTime(date)+"</span></li>")});
$("div.tumblr img.loader").css("display","none")});$("div.delicious img.loader").css("display","block");$.getJSON("http://feeds.delicious.com/v2/json/miguelmota/?count=3&callback=?",function(data){$.each(data,function(i,item){var title=item.d;var url=item.u;var date=(new Date(item.dt)).toUTCString();var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>
ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>1?"s":"")+" ago";return amount}}();$("ul.delicious_bookmarks").append("<li>&#187; <a href='"+url+"'>"+title+"</a> <span class='date'>"+niceTime(date)+"</span></li>")});$("div.delicious img.loader").css("display","none")});$("div.lastfm img.loader").css("display","block");$.getJSON("http://ws.audioscrobbler.com/2.0/?method=user.getRecentTracks&user=miguel_mota&api_key=b25b959554ed76058ac220b7b2e0a026&format=json&limit=5&callback=?",
function(data){$.each(data.recenttracks.track,function(i,item){var url=item.url;var name=item.name;var artist=item.artist["#text"];var date=item.date["#text"];$("ul.lastfm_tracks").append("<li><a class='link' href='"+url+"'>"+artist+" - "+name+"</a> <span class='date'>"+date+"</span></li>")});$("div.lastfm img.loader").css("display","none")});$("div.wakoopa img.loader").css("display","block");$.getJSON("http://api.wakoopa.com/miguelmota/recently_used.json?limit=3&callback=?",function wakoopaApi(data){var html=
["<ul class='wakoopa_software'>"];for(var i=0;i<data.length;i++){var entry=data[i].software;var date=(new Date(entry.last_active_at)).toUTCString();var niceTime=function(){var ints={second:1,minute:60,hour:3600,day:86400,week:604800,month:2592E3,year:31536E3};return function(time){time=+new Date(time);var gap=(+new Date-time)/1E3,amount,measure;for(var i in ints)if(gap>ints[i])measure=i;amount=gap/ints[measure];amount=gap>ints.day?Math.round(amount):Math.round(amount);amount+=" "+measure+(amount>
1?"s":"")+" ago";return amount}}();html.push("<li><a class='link' href='",entry.complete_url,"'>",entry.name,"</a> <span class='date'>"+niceTime(date)+"</span>","</li>")}html.push("</ul>");document.getElementById("wakoopa_software").innerHTML=html.join("");$("div.wakoopa img.loader").css("display","none")});c++;refreshStream()}}
function refreshStream(){setTimeout(function(){if(window.location.pathname.substr(1)=="stream"||window.location.pathname.substr(1)=="index"||window.location.pathname=="")$("a#stream").trigger("click")},3E4)}
function portfolioPage(){$("section.portfolio div.container").hover(function(){jQuery("div.overlay",this).fadeOut(300);$(this).css({"-webkit-box-shadow":"0 0 10px #fff","-moz-box-shadow":"0 0 10px #fff","box-shadow":"0 0 10px #fff"})},function(){jQuery("div.overlay",this).hide().fadeIn(300);$(this).css({"-webkit-box-shadow":"2px 2px 5px #111","-moz-box-shadow":"2px 2px 5px #111","box-shadow":"2px 2px 5px #111"})});$("a.fancybox").fancybox({padding:0,transitionIn:"fade",transitionOut:"fade",speedIn:600,
speedOut:200,overlayColor:"#000"})}
function contactPage(){$("form.contact_form a.submit").live("click",function(){$.validator.addMethod("namecheck",function(value,element){return this.optional(element)||/^[a-zA-Z]+?\s?[a-zA-Z]+?\s?[a-zA-Z]+$/.test(value)});$("form.contact_form").validate({rules:{name:{namecheck:true,required:true},email:{required:true,email:true},message:{required:true,minlength:10}},messages:{name:{namecheck:"invalid",required:"required"},email:{required:"required",email:"invalid"},message:"required",minlength:"required"},
onkeyup:true,debug:true});if($("form.contact_form").valid()==true){var str=$("form.contact_form").serialize();$.ajax({type:"post",url:"http://miguelmota.webuda.com/contact/mailer.php",data:str,success:function(){success()},error:function(){success()}});return false}else return false});function success(){$("form.contact_form").slideUp(300);setTimeout(function(){$("form.contact_form").html("<p>Thank you.<br />Your message has been successfully sent!<br />I will get in touch with you soon.</p>").fadeIn(1200)},
300)}}google.load("search","1",{language:"en"});function loadSearch(){var customSearchControl=new google.search.CustomSearchControl("013110027163283765539:a-en5rcxdcu");customSearchControl.setResultSetSize(google.search.Search.FILTERED_CSE_RESULTSET);customSearchControl.draw("cse")}function blogPage(){loadSearch();loadDisqus()};