$(document).ready(function(){if($.cookie("IE")==null)$.cookie("IE","1",{expires:7,path:"/"});if($.cookie("IE")=="1")$(".ie-notice-wrap").slideDown("slow");else $(".ie-notice-wrap").css("display","none");$(".ie-notice-close").live("click",function(){$(".ie-notice-wrap").slideUp("slow");$.cookie("IE","0")});textTickerSelected();$(".main-side-nav-ul a").each(function(){if($(this).attr("id")==$(".content").attr("class").split(" ")[1]){$("#stream").removeClass("selected");$(this).addClass("selected");
...
$(document).scrollTop();scrollTop=parseInt(scrollTop);var offset=topYloc+scrollTop+"px";$(".top-link").animate({top:offset},{duration:500,queue:false})});topYloc=parseInt($(".top-link").css("top").substring(0,$(".top-link").css("top").indexOf("px")));$(".icon-social").mtip()});$(window).scroll(function(){if(window.pageYOffset>=200)$(".top-link").fadeIn(600);if(window.pageYOffset<200)$(".top-link").fadeOut(185)});
function recent_tweets(data){for(i=0;i<1;i++){var date=new Date(data[i].created_at);document.getElementById("tweets").innerHTML+='<div class="tweet-content"><a href="http://twitter.com/miguelmota/status/'+ +(data[i].id_str?data[i].id_str:data[i].id)+'" rel="external">'+data[i].text+'</a> <time class="tweet-date">'+niceTime(data[i].created_at)+"</time></div>"}document.getElementById("tweet-wrap").style.display="block";$(".tweet-bird-icon").mtip();$("#tweet-wrap").live({mouseenter:function(){showMtip(".icon-twitter-bird-24",
this)},mouseleave:function(){hideMtip(".icon-twitter-bird-24",this)}})}$.getScript("http://miguelmota.tumblr.com/tweets.js");function loadMasonry(){var $tumblelog=$(".content");if(window.width<=640)$tumblelog="";$tumblelog.imagesLoaded(function(){$tumblelog.masonry({isFitWidth:true})})}
...
1)+" of "+currentArray.length+"</div>"}$(".fancybox").hover(function(){jQuery(".zoom-wrap",this).css("display","block")},function(){jQuery(".zoom-wrap",this).hide()})}
